...
with BeforeAndAfterAll {
  ...
  "A supervisor" should {
    "resume its child in case of a minor fault" in {
      ...
      child ! "Akka is awesome because I am learning to think in the whole new way"
      child ! Report
      expectMsg(3)
    }
  ...
  "An all-for-on supervisor" should {
    "apply the all-for-one strategy" in {
      val supervisor = system.actorOf(Props[AllForOneSupervisor], "allForOneStrategy")
      supervisor ! Props[FussyWordCounter]
      val child = expectMsgType[ActorRef]
...
object SupervisionSpec {
  ...
  class AllForOneSupervisor extends Supervisor {
    override val supervisorStrategy = AllForOneStrategy() { // apply strategy to all children
      case _: NullPointerException => Restart
      case _: IllegalArgumentException => Stop
      case _: RuntimeException => Resume
      case _: Exception => Escalate
